    ...
    function therapist_action_job() {
        ...
        if($this->exists_available_job($location, $user_id)) {
			...
			if($response == 'yes') {
				...
				$isSend = wp_mail($customerdata[44], "Your job has been accepted by a therapist", "Dear ".$customerdata[33].", your job has been accepted by a therapist, ".get_user_meta($user_id, 'first_name', true).", for That's The Spot Massage Therapy. For event details click <a href='".site_url('/my-events/?ev=c')."'>here</a>. ", '', false);
