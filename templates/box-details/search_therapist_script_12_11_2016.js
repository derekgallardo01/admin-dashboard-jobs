    ...
    var showResult = function(users) {
        ...
        if(users.length > 0) {
            ...
            for(var i in users) {
                ...
                ul.append('<li class="line-'+num+'"><label><input type="checkbox" name="therapists[]" class="found_therapists" value="'+user.ID+'" /><span>'+(user.full_name ? user.full_name : user.display_name)+'</span>'+address+'</label></li>');
