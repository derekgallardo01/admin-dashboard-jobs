...
<fieldset id="wrapper-payment-method" class="box-forms-send method-selc"><legend>1) <?php _e('Select the payment method:', $lang); ?></legend>
<div id="method-payment">
    ...
    </label>		<label title="<?php _e('Payment via Converge Pay', $lang); ?>" for="freshbooks" class="wimg">		<h2>Converge</h2>        <input class="radio-payment" id="converge" name="method_payment" type="radio" value="converge" />    </label>
</div>
</fieldset>
        ...
        <tbody>
            ...
            if($lead[Admin_Jobs::$field_id_corporate_massage]) {
                ...
                foreach ($participants_massage as $email) {
                    ...
                    ?>
                    <tr><td colspan="2"><b><?php _e('Client employee:', $lang)?></b>&nbsp;<?php echo $user->data->user_email; ?><!--<input type="text" name="client_email" id="client_email" value="<?php echo $user->data->user_email;?>" >--></td></tr>
                    <?php 
                ...
                <?php 				/* ?><tr>                    <td><button class="button upload_file_button" user="<?php echo $user_created->ID; ?>"><?php _e('Select File', $lang); ?></button></td>
                    <td><input id="path_file_<?php echo $user_created->ID; ?>" type="text" size="20" name="path_file_<?php echo $user_created->ID; ?>" value="" disabled="true" />&nbsp;<span class="valg-top"><?php echo $required; ?></span></td>                </tr>              <?php 			*/						} ?>
        </tbody>
    </table>
